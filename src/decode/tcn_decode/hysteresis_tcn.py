# hysteresis_tcn.py
#
# Step 3: Hysteresis decoding for full-sequence TCN predictions (after EMA)
#
# Input (per pid):
#   {in_dir}/{pid}/{out_tag}_probs_ema.npz
#
# Output (per pid):
#   {out_dir}/{pid}/{out_tag}_hysteresis_decoded.npz
...
def run_one(pid, args):
    # input path: in_dir/pid/out_tag_probs_ema.npz
    pid_in_dir = os.path.join(args.in_dir, pid)
    in_path = os.path.join(pid_in_dir, f"{args.out_tag}_probs_ema.npz")
    ...
    # output path: out_dir/pid/out_tag_hysteresis_decoded.npz
    pid_out_dir = os.path.join(args.out_dir, pid)
    os.makedirs(pid_out_dir, exist_ok=True)
    out_path = os.path.join(pid_out_dir, f"{args.out_tag}_hysteresis_decoded.npz")
    ...
    print(f"[OK] pid={pid} | saved={out_path} | T={probs.shape[0]} | frac_changed={frac_changed:.4f}")
...
def main():
    ...
    # file naming
    ap.add_argument("--out_tag", default="tcn", help="Must match EMA out_tag")
